...
const ExamBank = () => {
  ...
  const loadExams = async (page = pagination.page) => {
    try {
      ...
      if (
        ...
        filters.duration_from.trim() &&
        !isNaN(parseInt(filters.duration_from.trim()))
      ) {
      ...
      if (
        filters.duration_to &&
        filters.duration_to.trim() &&
        !isNaN(parseInt(filters.duration_to.trim()))
      ) {
      ...
      const errorMessage =
        ...
        "Có lỗi xảy ra khi tải danh sách đề thi.";
  ...
  const handleViewDetail = async (examId) => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message ||
        "Có lỗi xảy ra khi tải chi tiết đề thi.";
      toast.error(errorMessage);
    } finally {
      setLoading(false);
    }
              ...
              <div className="space-y-4 mb-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-black mb-2">
                    Tìm kiếm theo tên đề thi hoặc người tạo
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFilterChange("earliest_start_time", e.target.value)
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFilterChange("lastest_start_time", e.target.value)
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-black font-medium"
                  />
                </div>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 text-gray-600">
                ...
                <p className="text-sm mb-6">
                  Hãy thử thay đổi bộ lọc hoặc tạo đề thi mới.
                </p>
                <Link
                  to="/CreateExam"
                  ...
                  exams.map((exam) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {exam.list_question?.length || 0} câu hỏi •{" "}
                            {exam.duration} phút
                          </p>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm text-gray-700 mt-4 pt-4 border-t border-gray-200">
                        <div>
                          <span className="font-medium">Mở đề:</span>{" "}
                          {new Date(exam.earliest_start_time).toLocaleString()}
                        </div>
                        <div>
